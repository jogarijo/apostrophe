...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.pageNotFound = function(req, callback) {
      var partialUrls = req.url
        .replace(/\/+/, '/')
        .split(/(?!^)\//)
        .reduce(function (acc, segment) {
          return acc.concat((acc.length ? acc[acc.length - 1] + '/' : '') + segment);
        }, [])
        .reverse();

      return self.apos.docs.find(req, { historicUrls: { $in: partialUrls } }).sort({ updatedAt: -1 }).toObject(function(err, doc) {
        if (err) {
          return callback(err);
        }
        if (!doc) {
        ...
        if (self.local(doc._url) !== req.url) {
          var matchedHistoricUrl = (doc.historicUrls || []).find((url) => partialUrls.includes(url));
          var newUrl = req.url.replace(matchedHistoricUrl, doc._url);
          return req.res.redirect(statusCode, self.local(newUrl));
        }
        return callback(null);
      });
    };
